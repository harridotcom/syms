...
object GrpData {
    ...
    fun extractGroupedTransactionDetails(input: String): List<List<Pair<String, Double>>> {
        // Regular expression to match each "items" entry in the log
        val regex = Regex("items [^=]+=\\[([^]]+)]")
        val groupedTransactionDetails = mutableListOf<List<Pair<String, Double>>>()
        ...
        regex.findAll(input).forEach { matchResult ->
            ...
            groupedTransactionDetails.add(products) // Add the group to the main list
        }

        return groupedTransactionDetails
    }
